...
SELECT
    T.TEAM_NAME 팀명, P.PLAYER_NAME 선수명, P.POSITION 포지션, P.BACK_NO 백넘버, P.HEIGHT||'cm' 키
FROM PLAYER P
    JOIN TEAM T
        ON T.TEAM_ID LIKE P.TEAM_ID
        AND P.HEIGHT<
...
SELECT S.HOMETEAM_ID ID, S.STADIUM_NAME 경기장명
FROM STADIUM S
WHERE EXISTS (SELECT 1
            FROM SCHEDULE
            WHERE STADIUM_ID LIKE S.STADIUM_ID
            AND SCHE_DATE LIKE '201205%')
            ORDER BY S.STADIUM_NAME;
            
SELECT DISTINCT S.HOMETEAM_ID ID, S.STADIUM_NAME 경기장명
FROM STADIUM S
    JOIN (SELECT STADIUM_ID
            FROM SCHEDULE
            WHERE SCHE_DATE LIKE '201205%') SCH
    ON S.STADIUM_ID LIKE SCH.STADIUM_ID
ORDER BY S.STADIUM_NAME;